...
def main():
    ...
    for i in range(n_runs):
        ...
        sol_tok = tokenizer(solution_text, add_special_tokens=False, return_tensors="pt").to(model.device)
        ...
        print(f'solution text: {solution_text}')
        if target_str in output_text:
            print(f'Target acquired via original solution, prefix={solution_text}')
            memorized = True
        else:
            print(f'No match, output={output_text}')

        if memorized:
            match += 1
            match_list.append([seed, solution_text, optimized_ids])
        else:
            # do analysis.
            non_match_list.append([seed, solution_text, optimized_ids])
